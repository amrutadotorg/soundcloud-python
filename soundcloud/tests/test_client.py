...
def test_disabling_ssl_verification(mock_get):
    ...
    client = soundcloud.Client(
        client_id="foo", host="api.soundcloud.dev", verify_ssl=False
    )
    expected_url = "{}?{}".format(
        client._resolve_resource_name("tracks"),
        urllib.parse.urlencode({"limit": 5, "client_id": "foo"}),
    )
    headers = {"User-Agent": soundcloud.USER_AGENT, "Accept": "application/json"}
    mock_get.return_value = MockResponse("{}")

    client.get("tracks", limit=5)

    mock_get.assert_called_with(
        expected_url, headers=headers, verify=False, allow_redirects=True
    )
...
def test_method_dispatching_post_request(mock_post):
    ...
    mock_post.assert_called_with(
        expected_url, data=data, headers=headers, allow_redirects=True
    )
...
def test_proxy_servers(mock_get):
    ...
    expected_url = "{}?{}".format(
        ...
        urllib.parse.urlencode({"client_id": "foo"}),
    )
    headers = {"User-Agent": soundcloud.USER_AGENT, "Accept": "application/json"}
    mock_get.return_value = MockResponse("{}")
    ...
    mock_get.assert_called_with(
        expected_url, headers=headers, proxies=proxies, allow_redirects=True
    )
